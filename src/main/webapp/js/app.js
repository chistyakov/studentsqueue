...
myApp.controller('TodoCtrl', function($scope, dataService) {
    $scope.data = null;
    $scope.queues = [
      {id:3,name:'физика',expanded: false, items:
      ...
      {id:1,expanded: false, name:'Математика', items:[{id: 19,number: 12, fio: 'Miheev Dmitriy', description: 'qwe'}]}];
  ...
  $scope.addStudentToQueues = function() {
      ...
      $scope.queues.push({id:$scope.selectNewQueue.id,expanded: false,name:$scope.selectNewQueue.name, items:[{id:$scope.studentInfo.id, fio: $scope.studentInfo.fio, number: $scope.studentInfo.number, description:$scope.description}]});
